...
parser.add_argument("image_path", type=str,default="./Container/test_cdc/test_images",
                    help="The path of the input image.")
parser.add_argument("--anchor_path", type=str, default="./Container/container_anchors.txt",
                    help="The path of the anchor txt file.")
parser.add_argument("--new_size", nargs='*', type=int, default=[416, 416],
                    help="Resize the input image with `new_size`, size format: [width, height]")
parser.add_argument("--class_name_path", type=str, default="./Container/container.names",
                    help="The path of the class names.")
parser.add_argument("--restore_path", type=str, default="./Container/weight/container.ckpt",
...
for idx,f in enumerate(listdir(lists)):
    ...
    with tf.Session() as sess:
        ...
        for i in range(len(boxes_)):
            ...
            lines = [f ,int(labels_[i])+1,x0,y0,abs((x1-x0)),abs((y1-y0)) ,scores_ ]

            for idx,r in enumerate(lines): 
                submission.write(str(r))
                if idx != (len(lines)-1):
                    submission.write(",")
            submission.write("\n")           

    # cv2.imshow('Detection result', img_ori)
    cv2.imwrite(out_dirs + f, img_ori)
    # cv2.waitKey(0)
